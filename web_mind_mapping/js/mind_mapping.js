...
d3.json("bs_introduction/bs_introduction.json", function (err, tree) {
  ...
  function update(source, callback) {
    ...
    var node = svg_g.selectAll(".node").data(nodes, function (d) {
    ...
    node
      ...
      .attr("transform", function (d) {
        ...
        return (
          ...
          svg_x(d.y) +
          "," +
          (svg_y(d.x) - (d.x_size * scale - nodebox_vertical_margin) / 2) +
          ")"
        );
      });
    nodeEnter
      ...
      .attr("next", function (d) {
        if (d.children || d._children) {
          var $g = d3
            ...
            .attr({
              transform:
                "translate(" + (d.y_size - 6) + "," + (d.x_size / 2 - 5) + ")",
            })
            .on("click", click);
    ...
    node
      ...
      .attr("transform", function (d) {
        return (
          ...
          "," +
          (svg_y(source.x) -
            (source.x_size * scale - nodebox_vertical_margin) / 2) +
          ")"
        );
      })
      .remove();
    ...
    var link = svg_g.selectAll("path.link").data(links, function (d) {
      return d.target.id;
    });
  ...
  function parseText(text_tag, d) {
    ...
    if (typeof content === "string") {
      if (/^(\.\/)?img\//.test(content)) {
        ...
        $text
          .append("tspan")
          .attr({ x: "2", dy: "1.5em", path: content })
          .text("点击查看图片");
        d3.select(text_tag.parentNode)
          .attr("img_id", img_id)
          .on("click.show", function () {
            $modal
              .select("#" + d3.select(this).attr("img_id"))
      ...
      d3.select(text_tag.parentNode)
        .attr("code_id", content.id)
        .on("click.show", function () {
          var code = $iframe_doc.getElementById(
            d3.select(this).attr("code_id")
          ).innerHTML;
